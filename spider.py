...
def fetch(url: str, retries=3, encoding="utf-8") -> Optional[str]:
...
def fetch_all_artist_ids() -> List[str]:
    ids = set()
...
def load_existing_source_urls(filename: str) -> set:
    source_urls = set()
...
def parse_artist_desc_page(html: str) -> Optional[dict]:
...
def parse_song_page(html: str, song_id: str) -> dict:
    soup = BeautifulSoup(html, "html.parser")
